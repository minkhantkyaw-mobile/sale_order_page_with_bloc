                ...
                onPressed: () {
                  if (selectedProduct != null && selectedUnit != null) {
                    final newPricelist = ProductPriceListModel(
                      id: pricelist?.id ?? DateTime
                          .now()
                          .millisecondsSinceEpoch,
                      productId: selectedProduct!.id!,
                      unitId: selectedUnit!.id, // Add ! here
                      price: double.tryParse(priceController.text) ?? 0,
                    );

                    if (pricelist == null) {
                      context.read<ProductPricelistBloc>().add(
                          AddPricelist(newPricelist));
                    } else {
                      context
                          .read<ProductPricelistBloc>()
                          .add(UpdatePricelist(newPricelist));
                    }
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Text('Price: \$${pricelist.price}'),
                          trailing: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              IconButton(
